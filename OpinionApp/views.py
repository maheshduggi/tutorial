    ...
    if request.method == 'GET':
        ...
        with open("session.txt", "r") as file:
            for line in file:
                user = line.strip('\n')
        status_data = ''
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'opinionmining',charset='utf8')
        with con:
            cur = con.cursor()
...
def PostMyComment(request):
    ...
    if request.method == 'POST':
        ...
        db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'opinionmining',charset='utf8')
